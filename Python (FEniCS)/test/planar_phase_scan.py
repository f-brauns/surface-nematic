...
Run_n=20
Run_n_z0=1
wnt_strength_array=np.linspace(10,303.16,Run_n)
#wnt_strength_array=np.linspace(340,390,Run_n)
z0_array=np.linspace(19.0,49.0,Run_n_z0)
...
file_loc=path_to_cache+'/test_phase_range'
defect_csv_file_loc='../cache/data/raw_data/test7.csv'
defect_csv_file_loc_bare='../cache/data/raw_data/test7_bare.csv'
try:
    os.mkdir(file_loc)
except:
...
for j in range(0,Run_n_z0):
    ...
    z0=49.02#z0_array[j]#0.79*j+4.16#7.9#z0_array[j]

    for i in range(0,Run_n):

        #wnt_strength=-303.16+i*5.26#-(100+5.26*6*(i+1))#wnt_strength_array[i]
        wnt_strength=-1075-i*10.26#wnt_strength_array[i]#-(100+5.26*6*(i+1))#wnt_strength_array[i]
        ...
        #Init_val=Df.single_defect(x0=0,y0=0, charge_1=1,direction_angle_1=0)
