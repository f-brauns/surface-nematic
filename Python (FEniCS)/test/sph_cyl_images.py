...
Na2=1
R=6
bot_aspect_array=np.linspace(1,28,Na1)
height_array=np.linspace(0.01,30,Na2)
...
file_loc=path_to_cache+'/D_m3_4'#+'/wnt_strength_'+str(np.round(np.max(wnt_strength_array)))+'-'+str(np.round(np.min(wnt_strength_array)))+'_Range_'+str(np.round(np.max(z0_array)))+'-'+str(np.round(np.min(z0_array)))
...
#---------------------------- -------------------------------------------
    ...
    for i in range(0,1):
        ...
        d_fE=Expression((" x[0]/(a*a)", " x[1]/(b*b)", "(sqrt(x[2]*x[2]) < h ) ? 0: ((x[2]> 0)  ? (x[2]-h)/(c*c):(x[2]+h)/(d*d) )","0","0"),a=R,b=R,c=top_aspect*R,d=bot_aspect*R,h=height_fuc, degree=1)
        ...
        sigma=1#R*np.sqrt((1-z0**2/(R**2*alpha**2)))
        concentration_fuc,full_dir_val=Func_space.Gradient_new(sigma,1,alpha=(R+height_fuc))
        ...
        while condition_check<Target_num:
            ...
            if (N % plot_density)==0:
               ...
               # vs.plot_wnt_range(plot_index=1,z0=z0,alpha=alpha,R=R,title='Wnt Strength: '+str(wnt_strength),color='red',view_elev=view_elev, view_azim=view_azim,sub_fig=False,alpha_p=0.9)
                ...
                vs.plot_func_surface(field_cood=S_order, Ct_id=0,plot_index=1,view_elev=view_elev+10, view_azim=view_azim,title='Nematic on Sphere: Back',color_range=[0,1],set_range=True,bg_off=True,cbar=False,alpha_c=0.9,marker_size=marker_sizea)
                vs.save_fig(file_loc+'/a',plot_index, 200,name=str(index_num), transparent=True)

                vs=Visualization(mesh_cood,mesh_loc,Dir_mesh,R=R,row_num=1,column_num=1,w=10,h=10)
                vs.plot_wnt_range(plot_index=1,z0=z0,alpha=alpha,R=R,title='Wnt Strength: '+str(wnt_strength),color='red',view_elev=view_elev+10, view_azim=view_azim,sub_fig=False,alpha_p=0.2)
              ...
              #  vs.plot_wnt_range(plot_index=1,z0=z0,alpha=alpha,R=R,title='Wnt Strength: '+str(wnt_strength),color='red',view_elev=view_elev, view_azim=view_azim+azim_change,sub_fig=False,alpha_p=0.2)
                ...
                vs=Visualization(mesh_cood,mesh_loc,Dir_mesh,R=R,row_num=1,column_num=1,w=10,h=10)
               # vs.plot_quiver(_director=Dir_profile,Ct_id=2,plot_index=1,plot_density=1,title='Nematic on Sphere: Bot',view_elev=view_elev, view_azim=view_azim+45,color='black',vector_length=len_size*R,vector_width=1.5)
                vs.plot_func_surface(field_cood=concentration_fuc, Ct_id=0,plot_index=1,view_elev=view_elev, view_azim=view_azim+45,title='Nematic on Sphere: Bot',color_range=[0,1],set_range=True,bg_off=True,cbar=False,alpha_c=0.9,marker_size=marker_sizea,cmap='GnBu')
                ...
                vs=Visualization(mesh_cood,mesh_loc,Dir_mesh,R=R,row_num=1,column_num=1,w=10,h=10)
               # vs.plot_quiver(_director=Dir_profile,Ct_id=2,plot_index=1,plot_density=1,title='Nematic on Sphere: Bot',view_elev=view_elev, view_azim=view_azim+45,color='black',vector_length=len_size*R,vector_width=1.5)
                vs.plot_func_surface(field_cood=grad_mag, Ct_id=0,plot_index=1,view_elev=view_elev, view_azim=view_azim+45,title='Nematic on Sphere: Bot',color_range=[np.min(grad_mag),np.max(grad_mag)],set_range=True,bg_off=True,cbar=False,alpha_c=0.9,marker_size=marker_sizea,cmap='GnBu')
